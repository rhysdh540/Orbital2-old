...
public class Orbital2Main {
	...
	private static final ByteBuffer ttf; // Storage for font data
	...
	private static long win; // The identifier of the GLFW window
	...
	private static void render(int AA, int max_vertex_buffer, int max_element_buffer) {
		try (MemoryStack stack = stackPush()) {
			// setup global state
			glEnable(GL_BLEND);
